...
import { getTotalExpenses } from "./utilities.js"
...
export async function renderBalance(balanceTimeValue) {
	...
	const dollarBalanceString = formatter.format(
		balanceObject.transactionBalanceDollarValue
	)
...
export async function renderTransactions(getTransactionsData, preferences) {
	...
	data.forEach((item) => {
		...
		dollarValue.innerText = formatter.format(timeValueObject.amount)
...
export async function renderAggregatedCategoryTransactions(
	...
	data.forEach((aggregate) => {
		...
		const timeValueObject = timeValue(
			Math.abs(aggregate.value) / 100,
			preferences.rateObject
		)

		const currentCategoryID = aggregate.category
		const mappedName = categoryMap.find((item) => item.id == currentCategoryID)
		...
		const timeValueElement = elt.querySelector(".time-value")
		const dollarValue = elt.querySelector(".dollar-value")

		categoryName.innerText = mappedName.name
		timeValueElement.innerText = timeValueString
		dollarValue.innerText = formatter.format(Math.abs(aggregate.value) / 100)

		categoryList.appendChild(elt)
	})
}

export async function renderTotalExpenses(aggregatedTransactions, preferences) {
	const totalExpenses = await getTotalExpenses(aggregatedTransactions)
	const totalTimeBalance = document.querySelector(".balance-header.time-value")
	const totalDollarBalance = document.querySelector(
		".balance-header.dollar-value"
	)
	const currencyIndicator = document.querySelector(".currency-indicator")
	const timeValueObject = timeValue(totalExpenses / 100, preferences.rateObject)
	...
	totalTimeBalance.innerHTML = timeValueString
	totalDollarBalance.innerHTML = formatter.format(totalExpenses / 100)
}
